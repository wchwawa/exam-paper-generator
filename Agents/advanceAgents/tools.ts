...
export const generateExamQuestions = new DynamicStructuredTool({
  ...
  schema: z.object({
    ...
    short_answer_count: z.number().describe("short_answer questions count"),
    content: z.string().describe("week content"),
  }),
  func: async ({ topics, mcq_count, short_answer_count, content }) => {
    const prompt = `
    ## Role ##
    ...
    2. Provide ${mcq_count} MCQ quesitons and ${short_answer_count} short_answer questions, fully based on the given materials.
    ...
    6. Ensure the number of questions is align with the given number, ${mcq_count} for MCQ and ${short_answer_count} for short_answer.
...
export const checkQuestionsQuality = new DynamicStructuredTool({
  ...
  schema: z.object({
    ...
    short_answer_count: z
      .number()
      .describe("expected short_answer questions count"),
  }),
  func: async ({
    questions_json,
    week_num,
    ...
    try {
      ...
      const response = await llm.invoke(
        ...
        3. whether the short_answer questions have a clear reference answer
        4. whether the questions are of appropriate difficulty
        5. whether the number of multiple choice questions is ${mcq_count}
        6. whether the number of short_answer questions is ${short_answer_count}
...
export const responseFormatTool = new DynamicStructuredTool({
  ...
  description:
    "Before you return the final response, you need to format the response to the JSON format with this tool",
  schema: z.object({
